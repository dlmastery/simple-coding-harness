...
starts over. retryable(error) draws the line: a 4xx is never retried.
When the tries run out, call_llm returns a StreamedMessage whose `failed`
field carries the reason instead of raising, so the loop can show it and
go on. The rest is step 32: build_system_prompt lists the deferred tools;
PLAN_PROMPT and with_mode() are step 28; call_llm streams.
...
def build_system_prompt(cwd=None, schemas=None):
    ...
    so the prompt is built after the MCP tools have joined it.
    """
