...
`harness` command, one feature at a time. This step puts the human in the
loop: the `ask_user` tool, four answers at the approve prompt (`y`, `n`, `a`
for the session, `never` for the session) and Ctrl-C steering of a running
turn.
...
- From the repository root: `python run_tests.py 35` and
  `python check_snippets.py 35`.
...
- `harness/` - the package. `agent.py` is the loop, `llm.py` builds the
  ...
  `ask_user.py` is the question tool, `permissions.py` holds the session rules.
