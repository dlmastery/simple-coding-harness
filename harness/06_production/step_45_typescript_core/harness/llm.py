"""Step 43 - the tail of the system prompt comes from the registry:
extensions.prompt_sections() renders every section an extension
registered, in order. The skills paragraph is the first, from the
built-in skills extension; a project extension's paragraph follows it.
Before this step the template called skills_prompt() itself.
The rest is step 40: build_system_prompt takes the opening role as a parameter:
