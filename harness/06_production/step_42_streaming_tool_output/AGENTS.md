...
`harness` command, one feature at a time. This step adds streaming tool output: a command's lines reach a live panel
as they arrive (`harness/streaming.py`, `ui.streaming`), for foreground
commands, background jobs and subagent runs. The capstone of step 38 (a
FastAPI todo API run headless and graded by five checks) is still here, in
`capstone/`, and runs on this harness.
...
- From the repository root: `python run_tests.py 42` and
