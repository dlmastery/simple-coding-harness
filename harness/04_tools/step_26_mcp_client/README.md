...
You are in **Connect tools and observe the work**. Start with [setup](../../START-HERE.md) if you opened this lesson directly. Previous: [Step 25 - Persistent memory](../step_25_memory/README.md). Next: [Step 27 - Hooks](../step_27_hooks/README.md).
...
```bash
cd harness/04_tools/step_26_mcp_client
pip install -e ".[mcp]"
...
A tool arrives through MCP. Does that make its outputs trustworthy or its permissions correct?
...
No. The protocol exposes a capability. The application still needs appropriate authorization, input handling and output checks.
