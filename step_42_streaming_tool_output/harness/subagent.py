"""Step 42 - a long subagent run has a panel while it runs. loop()
...
def loop(system_prompt, request, tools, max_turns, label="subagent exploring", tag=None):
    ...
    """
    from .ui import ui  # imported here, not at the top: tools imports us
...
def turns(messages, tools, max_turns, label, tag, progress, report):
    """The turns of loop(): call the model, run its tools, until it answers or the cap is hit."""
...
def progress_line(name, args, result):
    """One line for the run's panel: the nested call and the size of what came back."""
    shown = str(next(iter(args.values()), "")) if len(args) == 1 else json.dumps(args)
    return f"{name} {title(shown)} · {len(result.splitlines())} lines"
...
def parallel(descriptions):
    ...
    reports = gather([partial(guarded, number, description) for number, description in enumerate(descriptions, 1)])
    return "\n\n".join(
