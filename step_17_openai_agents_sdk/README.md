...
agents-as-tools. The coding tools are ours again - the step 5 functions come
...
- **`function_tool(_bash, needs_approval=..., tool_input_guardrails=[...], timeout=60)`.**
  ...
  the JSON schema (compare `schema()` in step 5). Policy hangs off the same
...
- **Approval is a pause, not a callback.** When `needs_approval` returns
  ...
  another process - that is the design difference from step 11's inline
  prompt.
- **`shape_request` is steps 6, 7, 10 and 14 in one function.** The
...
- **A subagent is `agent.as_tool()`.** Its own `Agent` with its own tool
  ...
  output crosses back. The four rules of step 15, as arguments.
