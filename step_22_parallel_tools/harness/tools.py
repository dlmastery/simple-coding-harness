"""Step 22 - execute() splits into decide() and run(), and execute_all() runs the
tool calls of one reply side by side. Decisions stay on the calling thread, in
order; only the allowed calls go to the pool.
