"""Stage 15 - exploration subagents (video 41:57).

"A task tool hands a self-contained exploration question to a fresh agent
that has its own context window. We just do the call_llm function, because
it receives a list of messages and outputs a response, and also inputs the
tool set."

Four rules, and the code below is really just these:

  1. it starts from an empty history           - "none of the chat context
     with the user is ever shared with the sub agent"
  2. it holds every tool but four              - "minus task, write_todos,
     str_replace and write"; no recursion, one subagent deep
  3. it runs the same loop as the main agent   - call_llm, append, tools
  4. only its final message.content comes back - "none of the tool calls of
     the sub agent ever get returned to the main agent"
"""
