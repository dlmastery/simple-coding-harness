# Step 18 - The same harness on DeepSeek Harness (dsh)
...
├── src/rules.js                   the step 12 rules, in JavaScript
...
|-----:|----------|------------------|-------------------|
| 1-4 | the two loops | `core/agent-loop` plugin; `harness.run(prompt)` waits from inbox receipt to idle | runtime |
| 2-3, 7 | `bash`, read, edit, registry | `dsh-base`: persistent `bash` (`pwsh` on Windows), `read` / `write` / `edit`, `str_replace_editor` opt-in; `ctx.tools.register(defineTool(...))` for your own | runtime |
| 5 | UI | `on_notification` callback: the session event stream (`tool/call`, `tool/result`, `turn/end`) | us (`summarize`) |
| 6 | skills | `skill/` package: skill provider registry + loader tool | runtime |
| 8, 9 | late injection, freshness | `context/` request-context plugins; `agent.inject()` for durable context | runtime |
| 10 | JSONL sessions | append-only `SessionEvent` log under `<home>/sessions`; `session_id=` to continue | runtime; we list them |
| 11 | todos | `todo/` package (`todo_write`) | runtime |
| 12 | allow / ask / deny, sandbox | `tools/pre-execute` waterfall: return `{kind: 'deny'}` / `{kind: 'ask'}` or `next()`; `interaction/` handles asks; fs and subprocess *providers* are the sandbox seam | us (the plugin), runtime (mechanics) |
| 13 | compaction | `compaction/` capability + provider | runtime |
| 14 | subagents | `subagent/` capability; providers range from a child agent to delegating a turn to Claude Code or Codex | runtime |
...
- **`plugin/src/index.js`.** Fifteen lines: `ctx.on('tools/pre-execute',
  ...
  `next()` so the default policy still applies. This is step 12's
