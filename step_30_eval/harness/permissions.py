"""Step 29 - which tool calls need a human. A background job is rated by
the same bash rules as a foreground command; the job tools that only look
at a job, wait for it or stop it are allowed.
...
REQUIRED = {"bash": ("command",), "bash_background": ("command",), "write_file": ("path",), "str_replace": ("path",), "browser_open": ("url",)}
