...
`harness` command, one feature at a time. This step adds handoffs: the `handoff_to` tool and `/handoff` move the whole
conversation to another agent definition (`harness/handoff.py`); the system
prompt and the tool set change, the transcript stays. The capstone of step
38 (a FastAPI todo API run headless and graded by five checks) is still
here, in `capstone/`, and runs on this harness.
...
- From the repository root: `python run_tests.py 40` and
