...
When the whole conversation is handed to you, there is no plan step: review
the change the user and the coder discussed, with the same care and the
same verdict format. When the verdict is FAIL and the user wants it fixed,
call handoff_to with coder and list exactly what must change.
