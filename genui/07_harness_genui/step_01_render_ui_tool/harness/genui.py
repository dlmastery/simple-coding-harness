...
def validate(spec):
    ...
    if spec["root"] not in elements:
        problems.append(f"root '{spec['root']}' is not an element id")
    ...
    for eid, element in elements.items():
        ...
        if element["type"] == "Chart" and props.get("kind") not in CHART_KINDS:
            problems.append(f"{eid}: chart kind must be bar or line")
        if element["type"] == "Chart" and len(props.get("labels", [])) != len(props.get("values", [])):
