...
// The document with our CSP first in <head>. A CSP meta tag applies only to
// what comes after it, so it has to be first; any CSP the model wrote is
// removed, so the document cannot loosen ours. A fragment without <head>
// gets the meta tag in front of it.
