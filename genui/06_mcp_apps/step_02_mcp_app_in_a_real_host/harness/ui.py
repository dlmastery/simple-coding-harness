"""Step 02 - app() draws an MCP App's data as a text card. Step 24 - replay
draws an image message by its caption. The rest is steps 21 and 22: the
agent's text streams in live (stream_start, stream_delta, stream_end), the
spinner can be stopped early, headless() sends every panel to stderr so stdout
carries only the answer, and worker threads get APPROVE_LOCK and an Idle
spinner.
"""
...
class UI:
    ...
    def app(self, title, subtitle, body_lines):
        ...
        self.console.print(
            Padding(Panel(body, title=Text(f"app · {title}", style=f"bold {ACCENT}"), title_align="left", border_style=ACCENT, padding=(0, 1)), (1, 2, 0, 2))
        )
