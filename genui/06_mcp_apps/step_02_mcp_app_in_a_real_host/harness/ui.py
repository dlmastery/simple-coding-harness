"""Step 02 - app() draws an MCP App's data as a text card. Step 24 - replay draws an image message by its caption. The rest is step 21:
the agent's text streams in live (stream_start, stream_delta, stream_end), the
spinner can be stopped early, and headless() sends every panel to stderr so
