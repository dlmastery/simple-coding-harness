...
def wait_done(page):
    ...
    done = [e for e in page.evaluate("window.__events") if e.get("done")][-1]
    if done.get("error") or not done.get("valid"):
        raise RuntimeError(f"the turn failed: {done.get('error') or done.get('errors')}")
    return done
