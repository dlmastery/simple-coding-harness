...
An element may carry an `on` field, {"press": {"action": "refresh_numbers",
"params": {...}}}; the action must be one the catalog declares or one the
schema builds in (setState and friends), or the page would drop the press.
