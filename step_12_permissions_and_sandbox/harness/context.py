...
1. Some context changes every call (the time, the git branch, which files
   moved, the state of the plan). It cannot go in the system prompt.
...
The block now has three parts: <env> (step 8), <todos> (this step) and the
file-change <system-reminder> (step 9).
"""
...
def reminder():
    ...
    return {"role": "user", "content": env_block() + todos_note() + changes_note()}
