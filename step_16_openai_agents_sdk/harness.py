...
ours again, so the tool code from step 7 comes back unchanged and gets
...
# --- step 6: skills, exactly as before -------------------------------------------
...
# --- step 7: the tool implementations are plain functions ----------------------
...
# --- step 12: policy = a guardrail (deny) + needs_approval (ask) ---------------


def verdict_for(tool_name, args):
    """(verdict, reason) for any of our tools, from the step 12 rules."""
    if tool_name == "bash":
...
# --- step 3: registration is decoration ------------------------------------------
...
# --- step 14: a subagent is an agent used as a tool -------------------------------
...
# --- steps 8, 9, 11, 13: shape every request without touching the session ----------
...
def shape_request(data: CallModelData) -> ModelInputData:
    ...
    Two jobs from the hand-built harness, in one place:
      strip - older tool outputs shrink to a stub (step 13)
      inject - the late block goes on the end, and only on the wire (step 8)
...
# --- step 5: presentation is a RunHooks subclass ------------------------------------
...
# --- step 10: sessions are SQLite rows; rewind pops them ---------------------------
