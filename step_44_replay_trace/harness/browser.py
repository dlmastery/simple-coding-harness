...
_worker_lock = threading.Lock()  # two parallel subagents must not each start a browser thread
...
def page():
    ...
    if _page is not None and _page.is_closed():  # the window was closed by hand: start again
        try:
            _browser.close()
            _playwright.stop()
        except Exception:  # noqa: BLE001 - it is gone already; that is the point
            pass
...
def browser_close() -> str:
    ...
    try:
        ...
        _playwright = _browser = _page = None  # whatever the close did, the next call starts fresh
