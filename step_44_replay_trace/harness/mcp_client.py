"""Step 43 - MCP is an extension. Its tools arrive late: the servers
start when the chat starts, not when the registry is built, so apply(ctx)
keeps the context and registers the /mcp command, and register() adds
each server's tools through that context when connect_all() has started
the server. Before this step register() wrote into TOOLS and
TOOL_SCHEMAS by hand and commands.py held the /mcp listing.
The rest is step 26: MCP client: tools served by other processes, over stdio.
...
from . import extensions, history
...
def list_command(messages, arg=""):
    """/mcp: one line per configured server: name, status, and its tool names."""
