"""Step 32 - the browse tool set goes through active_schemas(). The rest
is step 23: the browser subagent.
...
def toolset():
    """The browser tool schemas plus read_file, nothing else, deferred ones as stubs."""
    from .browser import SCHEMAS
    from .tools import TOOL_SCHEMAS, active_schemas

    return active_schemas([s for s in SCHEMAS + TOOL_SCHEMAS if s["function"]["name"] in TOOLS])
