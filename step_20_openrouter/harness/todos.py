...
def validate(todos):
    """The reason a list is unusable, or None. Checked before anything is stored."""
    ...
    for i, todo in enumerate(todos):
        ...
        for key in ("content", "activeForm", "status"):
            if not isinstance(todo.get(key), str):
                return f"Error: item {i} is missing {key!r}"
...
def write_todos(todos):
    ...
    if problem:
        return problem  # the old list stays; the model reads why
...
def rebuild(messages):
    """Recover the plan from the last write_todos in a transcript (resume, rewind)."""
    TODOS.clear()
