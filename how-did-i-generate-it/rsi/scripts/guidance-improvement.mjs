...
export const improvementGuidance = {
  ...
  '09.02': {
    example:'A fixed improver first adds a row-alignment check to task skill v0, then adds an error-slice report to the retained child. Two task-skill generations now exist. If the improver’s diagnosis and selection procedure remained v0 throughout, this is repeated improvement under a fixed improver, even though its outputs changed twice.',
