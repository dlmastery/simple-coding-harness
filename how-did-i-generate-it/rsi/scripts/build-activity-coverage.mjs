...
related.set('01.05', {path: 'how-did-i-generate-it/rsi/validation/fresh-session-results/README.md', label: 'Two fresh agent contexts with one fit and a missing-task stop', gap: 'No parent conversation was supplied. The complete packet reproduced all 4,358 baseline predictions; 13,115 independent assertions pass. The missing-task context identified missing inputs and ran zero fits. Broad filename discovery exposed historical evidence paths to the complete context, but no historical scores or prediction contents were opened. Shared filesystem, host instructions, self-reported read ledgers, unmetered inference and skipped learner responses limit the claim. This establishes fixed reuse in one host, not adaptive improvement or cross-agent portability.'});
for (const [id,gap] of Object.entries({
...
for (const [id,gap] of Object.entries({
  ...
  '04.02':'All five requested relations have sentence explanations and an actual domain-check pass. Unknown and omitted-fact cases expose coverage boundaries. The original Mermaid plan rendered as stacked panels in the narrow GitHub view; a corrected illustrated guide refers to named views. The draft and publication finding are preserved. No claim of actual execution from a fact table or learner assessment follows.',
