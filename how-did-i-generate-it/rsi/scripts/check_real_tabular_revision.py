...
def check(stage):
    ...
    report["retained_loss"] = report[["parent_loss", "selection_loss"]].min(axis=1)
    ...
    for row in report.itertuples():
        lines.append(f"| {row.task} | {row.candidate} | {row.selection_score:.6f} | {row.parent_loss:.6f} | {row.selection_loss:.6f} | {'Keep candidate for further study' if row.candidate_minus_parent < -1e-12 else 'Retain parent'} |")
