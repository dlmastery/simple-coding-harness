...
for(const [i,l] of lessons.entries()) {
  const input=resolve(provenance,`rendered-v2/rendered-gallery-v2-${i+1}.png`);
  ...
  records.push(`| ${l.id} | ${i+1} | ${bytes.readUInt32BE(16)} × ${bytes.readUInt32BE(20)} | ${hash} |`);
}
writeFileSync(resolve(destination,'README.md'),`# Technical schematics\n\nOriginal course diagrams rendered with Mermaid CLI 11.17.0 on a white background. These are schematic explanations, not measured results or Imagen-generated illustrations.\n\nThe [source and review record](../../../how-did-i-generate-it/rsi/visuals/REVIEW.md) retains the first and revised galleries. Each published image maps to the revised gallery index below. Source code: [lesson-diagrams.mjs](../../../how-did-i-generate-it/rsi/scripts/lesson-diagrams.mjs).\n\n| Lab | Revised gallery index | Pixels | SHA-256 |\n|---|---:|---|---|\n${records.join('\n')}\n`);
// Mermaid CLI emits Windows separators in Markdown image targets. Normalize for GitHub.
for(const filename of ['rendered-gallery.md','rendered-gallery-v2.md']) {
