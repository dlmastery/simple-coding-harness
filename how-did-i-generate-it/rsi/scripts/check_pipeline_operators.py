...
def check(work):
    ...
    require("base_identities_unchanged", combined.iloc[:288][["task", "recipe", "status"]].equals(
        old[["task", "recipe", "status"]]))
    # CSV reserialization can round the last binary floating-point bit.
    require("base_scores_unchanged", np.allclose(combined.iloc[:288].score, old.score,
