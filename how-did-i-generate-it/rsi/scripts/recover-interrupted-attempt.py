...
COMMANDS = []
if not (OUT / "RECOVERY-STARTED.txt").exists() or (OUT / "RECOVERY-CONTINUED.txt").exists():
    raise RuntimeError("Expected the inspected failed recovery exactly once")
(OUT / "RECOVERY-CONTINUED.txt").write_text("Continue after observed exit-2 refusal; do not repeat that probe.\n",encoding="utf-8")
shutil.copyfile(Path(__file__), OUT / "recover-interrupted-attempt-v2.py")
...
observed = (OUT / "stale-lock-refusal-output.txt").read_text(encoding="utf-8")
require("REFUSED: Workspace busy or interrupted." in observed and "Exit: 2\n" in observed,
        "Missing actual stale-lock refusal; do not fabricate or repeat it")
require(digest(WORK / "trials.csv") == digest(OUT / "LEDGER-BEFORE.csv"),
        "Previous refusal changed original ledger")
...
lock_path.unlink()
command("running-record-refusal",common,2)
