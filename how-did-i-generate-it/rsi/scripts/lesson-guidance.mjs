...
import {researchGuidance} from './guidance-research.mjs';
import {scienceFrontierGuidance} from './guidance-science-and-frontier.mjs';
import {capstoneGuidance} from './guidance-capstone.mjs';
export const guidance = {
  ...improvementGuidance,
  ...researchGuidance,
  ...scienceFrontierGuidance,
  ...capstoneGuidance,
  '00.01': {
  ...
  '04.03': {
    outputs:[['RULES.md','States the target-derived-input, train-only-transform, and selection-versus-final invariants.'],['Six fact tables','Give one isolated valid case and one isolated violation for each rule.'],['RULE-TESTS.md','Pairs expected and actual verdicts for the six base cases, preserving all inputs and outputs.'],['Units-extension cases','Keep the extra implementation, one measurement with units, one without, and both observed verdicts separate from the base cases.']],
  ...
  '04.04': {
    ...
    outputs:[['Original teaching DOMAIN.md','Contains the well-formed leaked-input, final-fitted-scaler, and final-selection violations.'],['Failure report','Names all three semantic failures.'],['Corrected facts and recheck','Pass the supplied rules while preserving the original failed case and explaining which computations would need rerunning.'],['Renamed copy and third check','Rename the feature in both facts in a copy of the original failure. Keep its output showing that the derivation still triggers rejection.']],
