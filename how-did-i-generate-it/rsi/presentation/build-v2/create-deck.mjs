...
for(let i=0;i<matches.length;i++){
  ...
  if(visual)notes+='\n\nIllustration: '+github+(visual[1].startsWith('../') ? 'rsi/assets/' + visual[1].slice(3) : 'rsi/assets/illustrations/'+visual[1])+'\nThe figure explains a mechanism. Its illustrative cards do not establish an empirical result.';
...
for(const spec of slideSpecs){
 ...
 if(spec.image){
   const asset=(spec.image.startsWith('../') ? 'rsi/assets/' + spec.image.slice(3) : 'rsi/assets/illustrations/' + spec.image);
...
const finalPath=path.join(workspaceDir,'output/rsi-masterclass-review-v2.pptx');
