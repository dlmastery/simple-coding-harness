...
Current continuation: author diagrams, READMEs, and skills first, with periodic GitHub checkpoints; then complete the full verification pass. See AUTHORING-FIRST.md. All 101 labs have an editorial pass. Seventeen selected generated illustrations and thirty-seven output versions are retained. The latest set adds ModularRSI, meta-skill evolution, and saved-state coordination, with seven illustrated research-group introductions. Lab 10.17 now explicitly applies its pipeline to its own instructions and compares updater behavior within four fits. The initial main overview is the style reference. Next author dependency-graph recovery, fair improvement comparisons, and a capstone evidence map, then continue the remaining research-group and individual-lab illustrations. The evidence inventory remains 63 related mappings and 38 unmapped labs; mapping is not completion. Research remains 30 papers and five reports. Both two-generation improver revisions were rejected, and the local ScienceBuddy exercise does not train an LLM. The available generator is explicitly approved. No real learner, other native agent, GPU, or cluster validation is claimed. Older notes below record historical states.
