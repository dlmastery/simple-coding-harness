...
Current continuation: author diagrams, READMEs, and skills first, with periodic GitHub checkpoints; then complete the full verification pass. See AUTHORING-FIRST.md. All 101 labs have an editorial pass. Eleven selected generated illustrations and twenty-one output versions are retained; ScienceBuddy was redesigned and RSIAgent added after the quality correction. The initial main overview is the style reference. Next author AIDE² and ScientistTwo, then complete the remaining theme and method explanations. The evidence inventory remains 63 related mappings and 38 unmapped labs; mapping is not completion. Research remains 30 papers and five reports. Both two-generation improver revisions were rejected, and the local ScienceBuddy exercise does not train an LLM. The available generator is explicitly approved. No real learner, other native agent, GPU, or cluster validation is claimed. Older notes below record historical states.
