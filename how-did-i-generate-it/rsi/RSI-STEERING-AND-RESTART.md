...
Current active work: the user said “ok go” to the clean student-path walkthrough. Its first stages are executed and archived in `rsi/evidence/2026-09-20/clean-journey`. Continue from `validation/CLEAN-JOURNEY-RESULTS.md`. Source clone: `work/clean-journey-75e9988/course`; learner workspace: its sibling `rsi-work`. Do not upgrade that clone mid-experiment. The authoring tool now persists lesson budgets and passes 16 tests. Next: generated harness packages, bounded improver comparison, final claim audit, and remaining teaching/visual/source reviews.
