...
Current continuation: author diagrams, READMEs, and skills first, with periodic GitHub checkpoints; then complete the full verification pass. See AUTHORING-FIRST.md. All 101 labs have an editorial pass. Fourteen selected generated illustrations and twenty-nine output versions are retained. The latest set adds AIDE², ScientistTwo, and the fixed data science process, with five illustrated research-group introductions and visual teaching guidance in the tutor skill. The initial main overview is the style reference. Next author modular harness changes and task/meta-skill evolution, then complete the remaining core graph, system, measurement, and capstone explanations. The evidence inventory remains 63 related mappings and 38 unmapped labs; mapping is not completion. Research remains 30 papers and five reports. Both two-generation improver revisions were rejected, and the local ScienceBuddy exercise does not train an LLM. The available generator is explicitly approved. No real learner, other native agent, GPU, or cluster validation is claimed. Older notes below record historical states.
