"""Step 32 - the loop measures what it sends. Before every model call it
takes budget.breakdown() of the messages and passes the total to ui.usage,
which prints it next to the real prompt count. After the call budget.check()
turns the real count into a warning at 50% and 75% of the window, once each.
chat() builds the system prompt after the MCP servers have started, so the
list of deferred tools in it is complete, and on --resume it rebuilds the
set of loaded tools from the transcript. The rest is step 30: main() has
subcommands, and the turn loop is unchanged.
"""
