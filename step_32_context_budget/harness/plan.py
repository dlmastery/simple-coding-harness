"""Step 32 - load_tool joins the read-only tools, so a deferred tool can be
loaded in plan mode too. The rest is step 28: read first, propose a plan,
...
READ_ONLY = ("bash", "read_file", "read_skill", "task", "load_tool")  # offered in plan mode, in this order
...
def toolset():
    ...
    return [by_name[name] for name in READ_ONLY if name in by_name] + [SUBMIT_PLAN_SCHEMA]  # load_tool is added by active_schemas when needed
