...
CAPPED = "[output capped:"    # marker from cap(): the full text is on disk for this turn
TRIMMED = "[output trimmed:"  # marker from strip() and fit(): the rest is gone for good; stripping twice is a no-op
...
def strip(messages):
    ...
    model no longer needs in full" are the same set. A capped result is
    shrunk like any other: its temp file is gone with the turn anyway.
    """
    shrunk = 0
