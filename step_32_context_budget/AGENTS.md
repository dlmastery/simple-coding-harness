...
`harness` command, one feature at a time. This step adds the context
budget: `budget.py` estimates what the next request carries, `/context`
shows it, and tool schemas over `DEFER_OVER` tokens go out as stubs the
model loads with `load_tool` (`tools.py`).
...
- From the repository root: `python run_tests.py 32` and
...
- `harness/` - the package. `agent.py` is the loop, `llm.py` builds the
  ...
  `commands.py` handles `/` commands, `budget.py` is the estimate,
  `instructions.py` finds these files.
