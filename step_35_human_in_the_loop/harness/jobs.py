"""Step 29 - background jobs: shell commands that keep running while the
...
def job_wait(job_id: str, timeout: int = DEFAULT_WAIT) -> str:
    ...
    if isinstance(job, str):
        return job
    timeout = max(0, min(int(timeout or DEFAULT_WAIT), MAX_WAIT))  # a wait is bounded; call again for more
    try:
