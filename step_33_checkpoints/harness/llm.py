"""Step 32 - the system prompt lists the deferred tools, and call_llm offers
active_schemas() by default, so a deferred tool goes out as a stub until it
is loaded. deferred_section(schemas) names the deferred tools for one tool
set; build_system_prompt(cwd, schemas) places it after the tool guidance.
