"""Step 32 - the usage line carries the estimate. usage(stats, estimate)
prints the real prompt count with the estimate the harness made next to it,
so the two can be compared at a glance; with no real count, the estimate
stands alone. context() draws the breakdown /context asks for. The rest is
step 31.
"""
...
class UI:
    ...
    def banner(self, sandbox_name="none", mode="act"):
        ...
        self.console.print(Padding(Text(f"mode: {mode}  ·  sandbox: {sandbox_name}  ·  /plan  /act  /init  /sessions  /rewind  /undo  ·  alt-enter for a newline  ·  ctrl-d (ctrl-z then enter on Windows), ctrl-c or /exit to leave", style=MUTED), (0, 0, 0, 2)))
