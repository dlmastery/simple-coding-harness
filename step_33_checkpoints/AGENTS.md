...
`harness` command, one feature at a time. This step adds checkpoints:
`checkpoint.py` copies a file before an edit tool changes it, so `/undo`
and `/rewind` put the files back along with the transcript.
...
- From the repository root: `python run_tests.py 33` and
...
- `harness/` - the package. `agent.py` is the loop, `llm.py` builds the
  ...
  `commands.py` handles `/` commands, `checkpoint.py` keeps the file
  copies, `instructions.py` finds these files.
