"""Step 28 - every model call is made for the current mode. The tool set
comes from plan.toolset() and the system prompt from llm.with_mode(), both
read at call time, so an approval in the middle of a turn changes the very
next request.
"""
...
from .llm import SYSTEM_PROMPT, call_llm, entry, with_mode
