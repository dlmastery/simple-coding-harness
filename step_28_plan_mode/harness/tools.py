"""Step 28 - submit_plan joins the registry. It is in TOOLS, so it can run,
but not in TOOL_SCHEMAS: plan.toolset() offers it only in plan mode, next to
the read-only tools. The hook points from step 27 are unchanged.
...
def decide(tool_call, allowed=None):
    ...
    not offered to the hooks: the rules said no first.
    """
    name = tool_call.function.name
...
def settle(action, reason):
    ...
    prompts the user and runs only on yes. An `error` is already its own
    result.
    """
