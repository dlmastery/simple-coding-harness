...
**A concrete example.** In the saved author-guided example, improver v1 requires recomputing the candidate score. A later round reads that version and rejects a report that claims MAE 9.0 while its predictions imply 159.95. The trace shows that the new instruction ran. It does not show that the system invented the instruction autonomously or that v1 is generally better across tasks. In the later [public-tabular study](../../evidence/2026-09-22/tabular-comparison/README.md#follow-one-updated-skill-into-later-work), two successive research skills read the frozen revised updater, preserve their prior-skill hashes and govern four actual fits after the common probes. This supplies a real ML trace of later candidate-trial use. It still does not establish post-promotion deployment or repeated successful meta-generations. The [complete-researcher study](../../evidence/2026-09-22/nested-research-evaluation/README.md) then uses a promoted development researcher as the parent of a later rewrite. The same frozen I1 generates the second researcher, which executes searches on six reserved tasks. Its source and behavior are checked. This 624-attempt author study is optional evidence, not an increase to this lab budget.
