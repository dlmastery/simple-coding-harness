...
lesson's directory. The actor pack is `.claude/skills/adult-income-skills` (`P`); there is no verifier pack - the meta pack `.claude/skills/skill-memory-meta` (`M`) writes the memory; the curriculum is `../tasks/01_adult_income` ..
`../tasks/06_synth_shift_b`, in that order, and the exam is `../tasks/07_exam`.

## Boot order
1. This file. 2. `tools.md`. 3. `P/eval.md`: how the pack is judged. 4. `P/SKILL.md` and `M/SKILL.md`: the procedures you follow for each arm and for the meta visit. Re-read `P/skill-memory/manifest.yaml` before every memory arm: the meta pack may have added or updated a card, and the next run boots what the last one wrote.
