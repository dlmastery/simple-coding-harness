---
name: {{slug}}-verifier
description: Turn the fit log of one problem into memory cards for the actor pack {{slug}}. Generated from task.json by rsi-writer. Use after the actor's memory arm on a problem is frozen and scored; input is the log and the profile, nothing else.
metadata:
  type: workflow
  version: "2.0"
  rsi: "on"
...
actor pack is `.claude/skills/{{slug}}` (`P` below: its memory is where
your cards go); your own pack is `.claude/skills/{{slug}}-verifier`
