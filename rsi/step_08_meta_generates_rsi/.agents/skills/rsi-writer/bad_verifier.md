...
`.claude/skills/adult-income` (`P`: its `memory.json` is where your cards go); your own pack
...
Your input is `{recipe, val_score, error}` rows and the profile, from one helper.
