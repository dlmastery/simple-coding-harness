...
## The comparison
- Two arms per problem, same seed, same split, same budget of {{n_fits}} fits: the memory arm (this pack as it is) and the control arm (`--arm control --memory off`: `memory.json` not read, which is lesson 01's static walk).
