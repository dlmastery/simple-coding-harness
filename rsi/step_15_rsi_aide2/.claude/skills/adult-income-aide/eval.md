...
## The comparison
- Two arms per problem, same seed, same split, same helper, same budget of 24 fits: the versions the outer loop names (`v1`, `v2`) and, for the delete-the-file check, the control arm (`--arm control --memory off`: `memory.json` not read, which is lesson 01's static walk).
