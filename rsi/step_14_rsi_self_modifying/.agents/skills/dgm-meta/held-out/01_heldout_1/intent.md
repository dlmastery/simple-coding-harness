...
name: heldout_1
index: 1
title: "Held-out benchmark 1 (three clusters per class)"
role: pool
target: target
metric: roc_auc
budget_fits: 24
...
---
# Intent: Held-out benchmark 1 (three clusters per class)

## What to improve
ROC-AUC on a seeded table no curriculum or exam problem uses: the fixed benchmark every archived variant is scored on.

## Why
An archive of stepping stones needs one fixed yardstick; the held-out score of a variant never changes after it is measured.
