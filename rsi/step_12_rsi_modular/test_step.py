"""Lesson 12 - ModularRSI: the harness as five module files; two actors contrasted on a benchmark-disjoint pool; the one\ndiffering module patched in the loser behind the gate; the eval table never used for validation.
...
RUNS = HERE / "runs"
PACKS = ['actor-a', 'actor-b', 'adult-income-verifier', 'modular-meta']
INTENTS = ['pool/01_pool_trees_1/intent.md', 'pool/02_pool_trees_2/intent.md']
...
def test_two_actors_differ_in_exactly_one_module():
    a = {p.name: p.read_text(encoding="utf-8") for p in (SKILLS / "actor-a" / "modules").iterdir()}
    b = {p.name: p.read_text(encoding="utf-8") for p in (SKILLS / "actor-b" / "modules").iterdir()}
    assert sorted(a) == sorted(b) == ["agent_loop.md", "completion.md", "context.md", "observation.md", "tool_use.md"]
    assert [n for n in a if a[n] != b[n]] == ["context.md"]
    assert "Search policy: static" in a["context.md"] and "Search policy: obey-memory" in b["context.md"]


def test_the_pool_is_disjoint_from_the_curriculum():
    pool = sorted(p.name for p in (HERE / "pool").iterdir())
    assert pool == ["01_pool_trees_1", "02_pool_trees_2"]
    curriculum = {front_matter((RSI / "tasks" / d / "intent.md").read_text(encoding="utf-8"))[0]["data"].get("random_state") for d in
                  ("05_synth_shift_a", "06_synth_shift_b", "07_exam")}
    for d in pool:
        meta = front_matter((HERE / "pool" / d / "intent.md").read_text(encoding="utf-8"))[0]
        assert meta["role"] == "pool" and meta["data"]["random_state"] not in curriculum


def test_meta_patches_one_module_on_the_pool_only():
    meta, body = skill("modular-meta")
    assert meta["metadata"]["patches"] == ["modules/*.md"] and meta["metadata"]["approval"] == "gate"
    assert "The eval table (`../tasks/`) is never used" in body and "One module per patch" in body
...
def test_live_claude_code():
    """The recorded run: both actors on both pool tables with the verifier writing to each; contrast names context.md and a winner;
    the loser's module is patched behind the gate; no curriculum table was touched."""
    live_or_skip()
    reset()
    text = claude(readme_prompt())
    for actor in ("actor-a", "actor-b"):
        for task in ("pool_trees_1", "pool_trees_2"):
            s = state(actor, task, "memory")
            assert s["frozen"] is True and s["test_scored"] == 1, (actor, task)
        assert not any((RUNS / actor / t).exists() for t in ("adult_income", "breast_cancer", "wine", "digits", "synth_shift_a", "synth_shift_b", "exam"))
    contrasts = [r for r in rows(RUNS / "modular-meta" / "pool_trees_2" / "traces.jsonl") if r.get("event") == "contrast"]
    assert contrasts and contrasts[-1].get("module") in ("modules/context.md", "context.md", None)
    if contrasts[-1].get("module"):
        gate = [r for r in rows(RUNS / "modular-meta" / "pool_trees_2" / "traces.jsonl") if r.get("event") == "gate"]
        assert gate and gate[-1]["keep"] in (True, False)
        loser = contrasts[-1].get("loser") or ("actor-b" if contrasts[-1]["winner"] == "actor-a" else "actor-a")
        assert list((RUNS / loser / "versions").iterdir())
    assert tree(SKILLS) == tree(MIRROR)
    assert "context.md" in text or "not localised" in text
