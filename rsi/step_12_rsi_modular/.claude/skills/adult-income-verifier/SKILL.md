...
Run every helper through the Bash tool from this lesson's directory. The actor pack is the one named by `--pack`, `.claude/skills/actor-a` or `.claude/skills/actor-b` (`P`: its `memory.json` is where your cards go); your own pack
