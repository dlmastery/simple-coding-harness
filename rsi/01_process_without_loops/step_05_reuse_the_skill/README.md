...
**A concrete example.** A new session reading “repeat our successful run” lacks the decisions hidden in “our.” The [saved handoff](../../evidence/2026-09-20/clean-journey/01-05/HANDOFF.md) names the task and exact skill without supplying a score to imitate. Its actual reuse happened in the same author context. The [missing-task review](../../../how-did-i-generate-it/rsi/validation/saved-reuse-review/MISSING-TASK-REVIEW.md) shows what a reader should request when the direct task reference is removed: the skill still requires TASK.md, which supplies the target, metric and split. Finding that dependency differs from guessing its contents.
