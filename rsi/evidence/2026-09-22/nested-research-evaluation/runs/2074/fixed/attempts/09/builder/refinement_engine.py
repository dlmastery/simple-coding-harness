...
def build_recipe(schema, kind, template, factor, seed=41):
    ...
    if isinstance(learner, (ExtraTreesClassifier, ExtraTreesRegressor, RandomForestClassifier, RandomForestRegressor)):
        # A larger factor must not turn unlimited depth into a finite cap.
        # Only lower-capacity proposals introduce a cap for that template.
        depth_reference = learner.max_depth if learner.max_depth is not None else 8
        depth = None if learner.max_depth is None and factor > 1 else max(2, min(32, round(depth_reference*factor)))
        learner.set_params(max_depth=depth,
        ...
        leaves = None if learner.max_leaf_nodes is None and factor > 1 else max(7, min(63, round(leaf_reference*factor)))
        learner.set_params(max_leaf_nodes=leaves,
...
def constructor_description(value):
    ...
    if isinstance(value, BaseEstimator):
        cls = type(value)
        parameters = []
        for key, item in sorted(value.get_params(deep=False).items()):
            # These supported sklearn parameters are continuous quantities:
            # C=1 and C=1.0 denote the same setting. Do not normalize all numeric
            # parameters: min_samples_leaf=1 and =1.0 have different meanings.
            if key in {"C", "alpha", "epsilon", "gamma", "l2_regularization", "learning_rate", "tol"} and isinstance(item, Real) and not isinstance(item, bool):
                item = float(item)
            parameters.append((key, constructor_description(item)))
        return ("estimator", cls.__module__, cls.__qualname__, tuple(parameters))
