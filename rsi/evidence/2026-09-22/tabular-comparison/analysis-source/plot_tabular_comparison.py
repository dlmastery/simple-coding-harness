...
NAMES = {6: "Letter recognition", 23: "Contraceptive method choice", 31: "German credit",
         361235: "Airfoil noise", 361237: "Concrete strength", 361247: "Naval propulsion (simulated)"}
...
def main():
    ...
    for ax, (task, row) in zip(axes.flat, scores.iterrows()):
        ...
        ax.set_title(f"{NAMES[task]} · task {task}", loc="left", weight="bold", color="#203C55")
        ...
        for bar, value in zip(bars, values):
            if pd.notna(value):
                label = f"{value:.6f}" if value < .01 else f"{value:.4f}"
                ax.text(bar.get_x()+bar.get_width()/2, value, label, ha="center", va="bottom", fontsize=10)
