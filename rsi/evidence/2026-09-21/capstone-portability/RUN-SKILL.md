...
Use project-local Python. Invoke one `run` operation with explicit task, model, features, seed, hypothesis, and absolute workspace. Before the first fit, record how the lab's total allowance is allocated across its experiment workspaces. Pass this workspace's allocation through `--attempt-limit`; do not give every comparison arm the whole lab allowance. Later processes recover that frozen limit; do not increase it on resume. Reuse the original experiment when the lesson continues its selection history. A lab notes folder is not a fresh experiment budget. If following a retained skill, read it first and pass its path for a snapshot. Record in `DECISION.md` which instruction caused which choice. A snapshot alone proves only which file was recorded.
