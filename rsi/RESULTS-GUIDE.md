...
The resulting researchers are now being compared on six reserved datasets
against the parent, fixed portfolio and random search. This prospective
comparison is still running. Its final scores remain unopened until every
model choice is frozen and independently checked.
...
and the host shares access to all files. The question under test is whether
that changed improver produces a useful later researcher under this contract.
