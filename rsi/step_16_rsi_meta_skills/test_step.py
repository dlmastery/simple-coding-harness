"""Lesson 16 - MetaSkill-Evolve: task skills evolve every problem under the gate; the meta pack's own five role files\nevolve every k problems with the human; the improver has a version history you can diff.
...
RUNS = HERE / "runs"
PACKS = ['adult-income', 'adult-income-verifier', 'task-skills-meta', 'meta-evolver', 'adult-income-curriculum']
...
def test_five_roles_carry_the_numbers_the_fast_loop_uses():
    roles = sorted(p.name for p in (SKILLS / "task-skills-meta" / "roles").iterdir())
    assert roles == ["allocator.md", "analyzer.md", "evolver.md", "proposer.md", "retriever.md"]
    assert "Policy flip threshold: 2" in (SKILLS / "task-skills-meta" / "roles" / "allocator.md").read_text(encoding="utf-8")
    assert "Cards per visit: 3" in (SKILLS / "task-skills-meta" / "roles" / "proposer.md").read_text(encoding="utf-8")
    body = skill("task-skills-meta")[1]
    assert "`Policy flip threshold` (from `roles/allocator.md`)" in body and "`Cards per visit` (from `roles/proposer.md`)" in body


def test_two_timescales_two_gates():
    fast, slow = skill("task-skills-meta"), skill("meta-evolver")
    assert fast[0]["metadata"]["approval"] == "gate" and fast[0]["metadata"]["patches"] == ["SKILL.md", "schema.json", "memory.json"]
    assert slow[0]["metadata"]["approval"] == "human" and slow[0]["metadata"]["patches"] == ["roles/*.md"]
    config = front_matter((SKILLS / "meta-evolver" / "config.md").read_text(encoding="utf-8"))[0]
    assert config == {"meta": "on", "k": 3}
    assert "must be a multiple of `k`" in slow[1] and "never touches the actor" in slow[1]
    assert "never touch the verifier pack, `eval.md` or `roles/`" in fast[1]


def test_meta_pack_has_a_version_history():
    assert "runs/task-skills-meta/versions/gen_NNN/" in skill("meta-evolver")[1]
...
def test_live_claude_code():
    """The recorded run: task skills patched (or not) after every problem under the gate; the slow loop asked the human after
    problems 3 and 6 only; a role change landed only on `approve`, snapshotted under the meta pack's own versions/."""
    live_or_skip()
    reset()
    text = claude(readme_prompt())
    turns = 1
    while "approve / edit / reject" in text.lower() or ("approve" in text.lower() and "reject" in text.lower() and turns < 4):
        text = claude("approve", cont=True)
        turns += 1
    for task in TASKS:
        for arm in ("control", "memory"):
            assert state("adult-income", task, arm)["test_scored"] == 1, (task, arm)
    slow = [r for t in TASKS if (RUNS / "meta-evolver" / t / "traces.jsonl").exists()
            for r in rows(RUNS / "meta-evolver" / t / "traces.jsonl") if r.get("event") in ("propose", "apply")]
    ...
    versions = list((RUNS / "task-skills-meta" / "versions").iterdir()) if (RUNS / "task-skills-meta" / "versions").exists() else []
    assert len(versions) == len(applied) <= 2
