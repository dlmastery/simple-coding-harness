...
Every command below takes `--arm memory` (the default) or `--arm control`.

## Boot order
1. This file. 2. `tools.md`. 3. `schema.json`: the recipe space (`fields`), the budget and the static list (`recipes`). 4. `memory.schema.json`: what a card is.
