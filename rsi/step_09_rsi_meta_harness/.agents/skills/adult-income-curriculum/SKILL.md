...
`.claude/skills/adult-income-verifier` (`V`); the meta pack is the one the
user names - `.claude/skills/adult-income-meta` (approval: human) or
`.claude/skills/adult-income-meta-gate` (approval: gate) - (`M`); the curriculum is
...
1. For each curriculum task `T`, in order (01 .. 06):
   ...
   d. Meta visit: follow `M/SKILL.md` on `T` with `--visit <n>` (`n` = the problem's index). Under `approval: human` you stop and ask the user before the second `patch_pack.py` call; under `approval: gate` the script decides and you report the gate's numbers. With `{"meta": "off"}` in `M/config.json` the script proposes nothing (META_OFF).
