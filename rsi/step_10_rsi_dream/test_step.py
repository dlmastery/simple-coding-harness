"""Lesson 10 - Dream-RSI: the fit log replayed as a simulator ranks five search policies at zero fits; the winner is\nproposed as the actor's policy line behind the private gate; a saturated log is named, not hidden.
...
RUNS = HERE / "runs"
PACKS = ['adult-income', 'adult-income-verifier', 'adult-income-meta-dream', 'adult-income-curriculum']
...
def test_policies_md_names_five_and_the_actor_knows_them():
    text = (SKILLS / "adult-income-meta-dream" / "policies.md").read_text(encoding="utf-8")
    for name in ("static", "obey-memory", "random", "neighbours-of-top-3", "prefer-untried-family"):
        assert f"`{name}`" in text
    body = skill("adult-income")[1]
    assert "Search policy: static" in body and "`neighbours-of-top-3`" in body and "`prefer-untried-family`" in body


def test_rank_policies_spends_nothing_and_counts_unknowns():
    tools = (SKILLS / "adult-income-meta-dream" / "tools.md").read_text(encoding="utf-8")
    assert "No fit is spent: `fits_spent: 0`" in tools and "`unknown`" in tools and "`saturated`" in tools
    body = skill("adult-income-meta-dream")[1]
    assert "Zero fits" in body and skill("adult-income-meta-dream")[0]["metadata"]["patches"] == ["SKILL.md"]
    assert "at a tie the policy with more unknown picks ranks higher" in body


def test_only_the_policy_line_is_patched_behind_the_gate():
    meta = skill("adult-income-meta-dream")[0]["metadata"]
    assert meta["approval"] == "gate" and "`gate" in section(skill("adult-income-meta-dream")[1], "Procedure")
...
def test_live_claude_code():
    """The recorded run: six problems with a replay after each - zero fits spent by the meta pack - the winner landed or rolled
    back behind the gate, the curve and the exam."""
    live_or_skip()
    reset()
