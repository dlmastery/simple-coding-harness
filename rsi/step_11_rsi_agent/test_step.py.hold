"""Lesson 11 - RSIAgent: the planner chooses the next experiments by uncertainty (broad, then deep); the actor runs the plan\nand freezes its memory before the test; the verifier sees only the world result.
...
RUNS = HERE / "runs"
PACKS = ['adult-income-actor', 'adult-income-verifier', 'adult-income-planner', 'adult-income-curriculum']
...
def test_planner_states_the_uncertainty_rule():
    body = skill("adult-income-planner")[1]
    assert "u = (1 - success) + c / (n + 1)" in body and "Broad c: 2.0" in body and "Deep c: 0.25" in body and "Experiments per phase: 12" in body
    assert "every family is touched before any is repeated" in body and "the family with the most faults" in body


def test_actor_runs_the_plan_and_freezes_memory_before_the_test():
    body = skill("adult-income-actor")[1]
    procedure = section(body, "Procedure")
    assert "`freeze_memory" in procedure and procedure.index("`freeze_memory") < procedure.index("`score_test")
    assert "plan.json" in procedure and "`write_plan" not in procedure
    plan = json.loads((SKILLS / "adult-income-actor" / "plan.json").read_text(encoding="utf-8"))
    assert plan == {"phase": None, "c": None, "experiments": []}


def test_write_plan_contract_refuses_a_second_plan_per_phase():
    tools = (SKILLS / "adult-income-planner" / "tools.md").read_text(encoding="utf-8")
    assert "refuse a second plan in the same phase on the same problem" in tools and "exactly 12 `experiments`" in tools
...
def fits_of(task, arm="memory"):
    return [r for r in rows(RUNS / "adult-income-actor" / task / arm / "traces.jsonl") if "t" in r]


def test_live_claude_code():
    """The recorded run: on every problem the broad phase touches every family before the deep phase repeats one, the memory is
    frozen before score_test, the verifier writes for the next problem; the exam over five seeds with the memory frozen."""
    live_or_skip()
    reset()
    text = claude(readme_prompt())
    for task in TASKS:
        s = state("adult-income-actor", task, "memory")
        assert s["frozen"] is True and s["test_scored"] == 1 and s["memory"] == "frozen", task
        events = [r.get("event") for r in rows(RUNS / "adult-income-actor" / task / "memory" / "traces.jsonl")]
        assert events.index("freeze_memory") < events.index("score_test"), task
        ...
        assert state("adult-income-actor", task, "control")["test_scored"] == 1
    assert (RUNS / "adult-income-actor" / "curve.json").exists() and (RUNS / "adult-income-actor" / "exam.json").exists()
    exam = json.loads((RUNS / "adult-income-actor" / "exam.json").read_text(encoding="utf-8"))
