...
`.claude/skills/adult-income-verifier` (`V`); the meta pack is `.claude/skills/adult-income-planner` (`M`); the curriculum is `../tasks/01_adult_income` ..
`../tasks/06_synth_shift_b`, in that order, and the exam is `../tasks/07_exam`.

## Boot order
1. This file. 2. `tools.md`. 3. `P/eval.md`: how the pack is judged. 4. `P/SKILL.md`, `V/SKILL.md` and `M/SKILL.md`: the procedures you follow for each arm, for the verifier and for the meta visit. Re-read `P/SKILL.md` before every memory arm: a meta visit may have changed its `Search policy:` line, and the next generation boots what the last one wrote.
...
2. For each curriculum task `T`, in order (01 .. 06):
   ...
   b. Memory arm: follow `P/SKILL.md` with the default arm: open; the planner's broad plan; fit its 12; the planner's deep plan; fit its 12; FREEZE; `freeze_memory`; score once; scorecard.
