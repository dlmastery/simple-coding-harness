# Step 15 - The same harness on the Claude Agent SDK
...
over JSON. Every mechanism from steps 1-14 has a counterpart; the table says
...
|-----:|----------|------------------|-------------------|
| 1-4 | `messages` list, the two loops | `ClaudeSDKClient.query()` / `receive_response()` | SDK |
| 2-3 | `bash`, `read_file`, registry | built-in `Bash`, `Read`, `Glob`, `Grep`; custom tools via `@tool` + `create_sdk_mcp_server` | SDK, plus one `@tool` of ours (`run_tests`) |
| 5 | rich UI, usage line | message stream: `AssistantMessage`, `ToolUseBlock`, `ResultMessage.usage` / `total_cost_usd` | us (`show()`) |
| 6 | `.agents/skills`, `read_skill` | `.claude/skills/*/SKILL.md` via `setting_sources=["project"]`, `skills="all"`, built-in `Skill` tool | SDK; we ship the skill |
| 7 | `write_file`, `str_replace`, errors as results | built-in `Write`, `Edit`; tool errors are already results | SDK |
| 8 | late-injected `<env>` block | `UserPromptSubmit` hook returning `additionalContext` | us (`env_context`) |
| 9 | file freshness reminders | built in: Claude Code injects its own `<system-reminder>` on changed files | SDK |
| 10 | JSONL sessions, `/rewind`, `--resume` | sessions on disk; `list_sessions()`, `ClaudeAgentOptions(resume=...)`, `resume_session_at` | SDK; we wire `--resume`, `/sessions`, `/resume <id>` |
| 11 | `write_todos`, re-injected plan | built-in `TodoWrite`; the plan is re-injected by Claude Code | SDK |
| 12 | allow / ask / deny, OS sandbox | `PreToolUse` hook (`permissionDecision: "deny"`) + `can_use_tool` callback; `sandbox={"enabled": True}` | us (rules), SDK (sandbox) |
| 13 | cap / strip / fit / compaction | automatic compaction; `PreCompact` hook; `/compact` passes through as a prompt | SDK |
| 14 | `task` subagent | `agents={"explorer": AgentDefinition(...)}` used by the built-in `Task` tool | us (definition), SDK (loop) |
...
- **Two-layer policy survives.** `deny_dangerous` is a `PreToolUse` hook:
  ...
  the same `rules.py` as step 12. The SDK gives you both hooks; it does not
...
- **Late injection is a hook.** The `UserPromptSubmit` hook returns
  ...
  cache reasoning as step 8, one function instead of a module.
- **A subagent is data.** `AgentDefinition(tools=[...], maxTurns=12)` is the
  four rules of step 14 as fields: fresh context, withheld tools, same loop,
