...
Everything steps 1-14 built by hand is built in here: the loop, Bash / Read /
...
# --- step 5/6/11: the prompt. The claude_code preset already carries the ----
...
# --- step 14: a subagent is a definition, not a loop --------------------------
...
# --- step 3: one custom tool, registered as an in-process MCP server ----------
...
# --- step 12: policy, still two layers ----------------------------------------
...
# --- step 8: late injection is a UserPromptSubmit hook ------------------------
# Claude Code already injects its own <system-reminder> blocks for changed
# files (step 9) and the todo list (step 11). This adds our <env> block.
...
def build_options(resume=None):
    options = ClaudeAgentOptions(
        ...
        cwd=os.getcwd(),
        setting_sources=["project"],  # step 6: loads .claude/skills/*/SKILL.md
        skills="all",
    ...
    if sys.platform != "win32":  # step 12: the OS sandbox is one flag here
