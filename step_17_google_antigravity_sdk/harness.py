...
# --- step 12: policy as data. `enforce()` compiles this into a decide hook. -------
...
# --- step 3: two tools of our own; the runtime derives schemas from the hints -----
...
# --- step 14: the subagent is a config object -------------------------------------
...
def build_config(resume=None, handler=confirm):
    return LocalAgentConfig(
        ...
        subagents=[EXPLORER],
        skills_paths=[str(Path.cwd() / ".agents" / "skills")],  # step 6
        workspaces=[str(Path.cwd())],
        save_dir=str(HOME),                                     # step 10
        conversation_id=resume,
        session_continuation_mode=SessionContinuationMode.RESUME if resume else None,
    )


# --- step 8: the late block is prepended by the caller, not stored anywhere -------
