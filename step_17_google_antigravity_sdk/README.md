# Step 17 - The same harness on the Google Antigravity SDK
...
|-----:|----------|-----------------|-------------------|
| 1-4 | the two loops | `async with Agent(config)`, `await agent.chat(prompt)` | runtime |
| 2-3, 7 | `bash`, read, write, edit, registry | built-ins `run_command`, `view_file`, `edit_file`, `create_file`, `search_directory`, `find_file`, `list_directory`; custom tools are plain Python functions in `tools=[...]` | runtime, plus `run_tests` and `write_todos` of ours |
| 5 | UI, usage | streamed tokens (`async for token in response`), `response.usage_metadata`, `@post_tool_call` for tool lines | us |
| 6 | skills | `skills_paths=[".agents/skills"]` | runtime |
| 8 | late injection | prepended to the prompt by the caller (`late_block()`); the runtime keeps its own context | us |
| 9 | file freshness | runtime-managed context | runtime |
| 10 | sessions, `--resume` | `save_dir`, `conversation_id`, `session_continuation_mode=RESUME` | runtime; we pass the id |
| 11 | todos | no built-in; `write_todos` tool + re-injection in `late_block()` | us |
| 12 | allow / ask / deny, sandbox, timeout | policy list (`allow` / `ask_user` / `deny` with `when=` predicates) compiled by `enforce()`; `@pre_tool_call_decide` hook; `RunCommandConfig(timeout_seconds, enable_sandbox)` | us (rules), runtime (mechanics) |
| 13 | cap / strip / fit / compaction | runtime compaction (`compaction_threshold`), `@on_compaction` hook | runtime |
| 14 | `task` subagent | `SubagentConfig` with `SubagentCapabilities(enabled_tools=read-only)`; built-in `start_subagent` | us (definition), runtime (loop) |
...
- **Policy is a list, not code.** `deny("run_command", when=is_denied)`,
  ...
  `allow("run_command", when=is_plain)`. The predicates call the step 12
  `rules.decide`. `enforce()` compiles the list into a decide hook; the
  offline test compiles it and checks the same verdicts as step 12.
...
- **A subagent is a config object.** `SubagentConfig(capabilities=
  ...
  step 14's four rules as fields: it cannot edit and it cannot recurse.
