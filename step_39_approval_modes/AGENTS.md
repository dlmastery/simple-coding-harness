...
`harness` command, one feature at a time. This step adds approval modes: `--mode` and `/mode` pick default,
accept-edits, read-only or auto, a table in `harness/modes.py` that rewrites
the verdict of the permission rules before a call runs. The capstone of step
...
- From the repository root: `python run_tests.py 39` and
